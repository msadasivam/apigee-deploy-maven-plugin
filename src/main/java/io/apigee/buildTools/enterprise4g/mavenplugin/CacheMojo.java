...
public class CacheMojo extends GatewayAbstractMojo
{
    ...
    public static String CACHES_FILE   = "caches.list";	
    ...
    private void createCachesUsingListFile() throws IOException, MojoFailureException, Exception {
        ...
        File cacheListFileDirectory = listFile.getParentFile();
        List<String> cacheFiles = FileUtils.readLines(listFile, null);
        processCaches(cacheFiles, cacheListFileDirectory);
    }

    private void processCaches(List<String> cacheFiles, File cacheListFileDirectory) throws IOException, MojoFailureException, Exception {
        for (String cacheFileString : cacheFiles) {
            if (isCommentOrBlank(cacheFileString)) {
                continue;
            }
            ...
            if (cacheFile.isAbsolute()) {
                ...
                doUpdate(new File(cacheListFileDirectory, cacheFileString));
            }
        }
    }

	// private File findCacheConfig(Logger logger) throws MojoExecutionException {
	 ...
	 * create Cache values
	 */
	protected void doUpdate(File cacheConfig) throws IOException, MojoFailureException, Exception {
		try {
			
			logger.info("\n\n=============Creating Cache================\n\n");
			// state = State.IMPORTING;
			bundleRevision = RestUtil.createCache(super.getProfile(), cacheConfig);
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		try {
			...
			createCachesUsingListFile();
			// File cacheConfig = findCacheConfig(logger);
			// doUpdate(cacheConfig);
